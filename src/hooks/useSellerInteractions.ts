...
export function useSellerInteractions(leadId?: string) {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const queryKey = ['seller_interactions', leadId];
  ...
  const addInteraction = useMutation({
    mutationFn: async (params: {
      ...
      const { error } = await supabase.from('seller_interactions').insert({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['leads', 'seller'] });
    },
    onError: (error: any) => {
