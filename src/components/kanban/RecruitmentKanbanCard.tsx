...
const expColors: Record<string, string> = {
  com_experiencia: 'bg-green-100 text-green-700 border-green-200',
  sem_experiencia: 'bg-sky-100 text-sky-700 border-sky-200',
};

export function RecruitmentKanbanCard({ lead, columns, isDragging, onClick, onMove, currentUserId }: Props) {
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className={`rounded-lg border bg-card p-3 space-y-2 cursor-grab active:cursor-grabbing transition-shadow hover:shadow-md ${isDragging ? 'opacity-50' : ''} ${interviewStaleAlert ? 'border-orange-400' : ''}`}
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {lead.experienceLevel && (
          <Badge variant="outline" className={`text-[10px] shrink-0 ${expColors[lead.experienceLevel] || 'bg-muted text-muted-foreground border-border'}`}>
